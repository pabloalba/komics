...
public class EdgeDetector {
	...
	private static boolean isBorderColor(int rgb) {
		...
		boolean black = ((((BLACK_RED - RANGE) < red) && (red < (BLACK_RED + RANGE)))
				&& (((BLACK_GREEN - RANGE) < green) && (green < (BLACK_GREEN + RANGE))) && (((BLACK_BLUE - RANGE) < blue) && (blue < (BLACK_BLUE + RANGE))));

		boolean white = ((((WHITE_RED - RANGE) < red) && (red < (WHITE_RED + RANGE)))
				&& (((WHITE_GREEN - RANGE) < green) && (green < (WHITE_GREEN + RANGE))) && (((WHITE_BLUE - RANGE) < blue) && (blue < (WHITE_BLUE + RANGE))));

		return black || white;

	}

	public static ArrayList<Scene> processImage(Drawable drawable,
		...
		for (int i = 0; i < horizontalLines.size() - 1; i++) {
			...
			for (int x = tenPercentWidth; x < width - tenPercentWidth; x++) {
				boolean blackLine = true;
				for (int y = horizontalLines.get(i); y < horizontalLines
						.get(i + 1); y++) {
					if (!isBorderColor(img.getPixel(x, y))) {
						blackLine = false;
						break;
		...
		for (int i = 0; i < zones.size(); i++) {
			...
			for (int j = 0; j < zoneLines.size() - 1; j++) {
				...
				if (deltaX > deltaY) {
					deltaY = (screenHeight * deltaX) / screenWidth;
				}
